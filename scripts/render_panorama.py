...
def _argparse():
    parser = argparse.ArgumentParser(
        description="Rohbau3D Panorama Rendering Script")
    parser.add_argument(
        "--data",
        type=str,
        required=True,
        help="Path to the directory which contains the site or scan folders")
    parser.add_argument(
        "--output",
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--features",
        default="all",
        help="Features list to render: all, depth, color, intensity, normal, class, instance")
    parser.add_argument("--upscale", type=int, default=4,
                        help="Upscale factor for rendering")
    parser.add_argument("--crop", type=bool, default=True,
                        help="Whether to crop the output images")
...
def process_folder(folder_info, args):
    ...
    try:
        ...
        classes = None
        print(
            f"Failed to load class.npy in {folder}, class image will not be rendered.")
    try:
        instance = np.load(join(folder, "instance.npy"))
    except BaseException:
        instance = None
        print(
            f"Failed to load instance.npy in {folder}, instance image will not be rendered.")
    ...
    if coord is not None and "depth" in feature_selection:
        img_depth = renderer.depth_image(
            normalize=True, upscale=args.upscale, crop=args.crop)
        img_depth.save(join(output_dir, f"depth.png"))

    if intensity is not None and "intensity" in feature_selection:
        img_intensity = renderer.intensity_image(
            upscale=args.upscale, crop=args.crop)
    ...
    if normal is not None and "normal" in feature_selection:
        img_normal = renderer.normal_image(
            upscale=args.upscale, crop=args.crop)
    ...
    if instance is not None and "instance" in feature_selection:
        img_instance = renderer.instance_image(
            upscale=args.upscale, crop=args.crop)
...
def main():
    ...
    # traverse the input dir and its subdirectories to list all folders
    # starting with "scene"
